...
- 用户选定英伦复古拟物风格：纸张、皮革、旧黄铜、实体边缘厚度与克制投影；工具栏、导航、弹层和按钮使用共享主题的凸起/内凹层次，不再默认采用扁平无阴影样式。保持画布操作区紧凑，不遮挡作品。
- 左侧画布面板等列表里的节点/元素缩略图容器，非图片类型（文本、配置、视频、音频等）不要使用 `theme.node.fill`（`#e7e5df`/`#292524`）这类灰色背景，图标直接无背景展示，尽量不要给多余底色，保持干净。
- 主要操作按钮使用可辨识的实体边缘、按压反馈和复古衬线字；次要图标操作保持轻量。选中态用内凹或主题高亮表达，避免霓虹渐变与持续循环动效，并支持减少动态效果。
