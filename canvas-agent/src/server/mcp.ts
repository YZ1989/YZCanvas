...
import { AGENT_PROMPT, loadConfig, type YZCanvasAgentConfig, VERSION } from "../config.js";

type YZCanvasAgentToolResponse = { ok?: boolean; result?: unknown; error?: string };
...
/** 向 MCP Server 注册单个 YZCanvas Agent 工具。 */
function registerCanvasTool(server: McpServer, config: YZCanvasAgentConfig, name: ToolName) {
...
/** 将 MCP 工具调用转发到本地 YZCanvas Agent HTTP 服务。 */
async function postCanvasAgentTool(config: YZCanvasAgentConfig, name: ToolName, input: unknown) {
    ...
    const body = (await res.json()) as YZCanvasAgentToolResponse;
