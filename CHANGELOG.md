...
+ [调整] 工作台升级为复古拟物界面：皮革黄铜新建按钮、烫金感宋体与按压反馈，统一导航、卡片、搜索和工具浮层的立体层次。

+ [优化] 缩小画布项目卡片，增加悬停透视倾斜、抬升和阴影反馈，系统减少动态效果时禁用动效。

+ [调整] 工作台采用英伦电影档案馆视觉：暖纸色、酒红色、摄影机静物与胶片边框，更新首页标题并统一深浅主题、设置和画布配色。

+ [调整] 工作台改为左侧导航、创作插画横幅与画布卡片布局，加入画布搜索，将账号和工具固定在侧栏底部。

+ [新增] 左下角 Jinyu API Key 登录与账户面板，集成钱包刷新、设置和退出；密钥验证成功后保存，保留本地画布。

+ [新增] Windows 双击启动浏览器测试版，自动复用本地开发服务并支持代码热更新，无需打包。

+ [新增] 补充 Jinyu G v2.5、Grok 2 编辑和两款 Omni 低价视频模型，恢复可灵、Vidu、Wan、HappyHorse 模型分组并更新参数约束。

+ [调整] 使用原创 YZ 画布图标替换桌面与应用标识，启动页改为 0.9 秒的轻量入场动画并支持减少动态效果。

+ [调整] 默认服务切换至 api.yz-jinyu.com，同步 Jinyu 模型、价格、余额、上传和桌面权限，增加文档入口并隔离旧服务密钥。

+ [修复] 统一文档站、默认渠道、下载文件和开发工具的 YZCanvas 命名，并修正首页主题存储键；保留上游来源与工程兼容标识。
